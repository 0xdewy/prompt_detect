...
if ! python3 -c "import torch, transformers, fastapi" 2>/dev/null; then
    ...
    if command -v uv &> /dev/null; then
        ...
        uv pip install -r "$SCRIPT_DIR/requirements_simple_production.txt"
    else
        echo "Using pip..."
        pip install -r "$SCRIPT_DIR/requirements_production.txt"
    fi
...
MODEL_DIR="$PROJECT_ROOT/prompt_detective/models/checkpoints"
if [ -d "$MODEL_DIR" ]; then
